...
from random import randint #random insecure, best use os urandom instead

ALPHABET = "abcdefghijklmnopqrstuvwxyz"

def generate_otp(sheets, length): 
#generate otp function taking num of sheets in pad & num of characters
#that each sheet can enctrypt as parameters
	for sheet in range(sheets):
#creates a new file for every sheet
		with open("otp" + str(sheet) + ".txt","w") as f:
			for i in range(length):
				f.write(str(randint(0,26))+"\n")

#now load a sheet and store all its nmbers in a list
